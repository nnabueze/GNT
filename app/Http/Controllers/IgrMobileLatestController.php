...
class IgrMobileLatestController extends Controller
{
    use Helpers;
        //Login User
    public function authentication(Request $request)
    {
        ...
        if (Auth::attempt(['email' => $request->input("email"), 'password' => $request->input("password")])) {
            ...
            $data['yestarday_remitted'] = 0;
            $data['today_remitted'] = 0;
            ...
            foreach ($igr->mdas as $mda) {
                ...
                $last_month_remitted = Remittance::where("mda_id",$mda->id)->where("remittance_status",1)->whereDate("created_at",">=", $firstday_last_date)
                                ->whereDate("created_at","<=",$lastday_last_date)->get();

                				$data['last_month_remitted'] = $last_month_remitted->sum('amount');


                $current_month_remitted = Remittance::where("mda_id",$mda->id)->where("remittance_status",1)->whereDate("created_at",">=", $firstday_curent_date)->get();
