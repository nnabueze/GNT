...
class IgrEbillsApiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($json['Step'] == 2 && $param['page'] == 6) {
            $item = $this->tax_step_2($json);
            return $item;
        }

        //tax collection
        if ($json['Step'] == 3 && $param['page'] == 6) {
    ...
    private function tax_step_2($param)
    {
        //getting param
        $data['BillerID'] = $param['BillerID'];
        ...
        //checking missing param
        if (empty($data['ercasBillerId']) || empty($data['Tin']) || empty($data['name']) || empty($data['phone']) || empty($data['mda'])) {
        ...
        $data['ResponseCode'] = "00";
            ...
            $content = view('xml.tax_step2', compact('data'));
